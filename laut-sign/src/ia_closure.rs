...
impl Walker {
    ...
    fn compute_pass1(&mut self, path: &str) -> Result<StorePath<String>, Error> {
        ...
        // `expected_non_self` is what nix-store reports as `path`'s runtime
        // references *excluding* self. The wu-manber scan below must observe
        // exactly this set. Divergence here means either the on-disk content
        // was tampered with (the recorded refs are stale) or the recorded
        // refs were generated by a scanner that disagrees with ours.
        //
        // Self-references are intentionally excluded from the strict equality
        // check: Nix records self-refs conservatively on some paths (e.g.
        // fetchurl FODs like busybox) even when the content doesn't literally
        // contain the path's own hash. Pass-1's self_mask handles the
        // "found or not" case correctly on its own.
