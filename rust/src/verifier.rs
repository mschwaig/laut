//! Trust-model verification for laut, based on the semantics agreed in the design discussion.
//!
...
impl TrustModel {
    ...
    pub fn validate(&self) -> Result<HashSet<KeyId>, String> {
        ...
        match self {
            ...
            TrustModel::Threshold(t, children) => {
                let has_legacy = children.iter().any(|c| matches!(c, TrustModel::KeyLegacy(_)));
                if has_legacy && *t != 1 {
                    return Err(format!(
...
impl Facts {
    ...
    pub fn add_claim(&mut self, rdrv: RDrv, signer: KeyId, output_map: HashMap<OutputName, ContentHash>) {
        self.rdrv_claims
            .entry(rdrv)
            .or_default()
...
impl<'a> Verifier<'a> {
    ...
    pub fn verify(&mut self, target_udrv: UDrv, target_subset: Subset) -> VerifyResult {
        ...
        result.reachable.insert((target_udrv, target_subset.clone()));
        ...
        while let Some((udrv, subset)) = worklist.pop() {
            ...
            for &rdrv in rdrvs {
                ...
                let dep_subsets = self.facts.rdrv_dep_subsets.get(&rdrv).unwrap_or(&empty_deps);
        ...
        let target_covered =
            self.facts.fods.contains_key(&target_udrv) || result.evidence.contains_key(&target_udrv);
...
mod tests {
    ...
    fn multi_output_target_subset_ignores_dev_divergence() {
        ...
        assert!(result.verified, "target ignores $dev, both keys agree on $out");
    }
    ...
    fn legacy_key_skips_upstream() {
        ...
        let mut v = Verifier::new(&facts, &tm).unwrap();
        let result = v.verify(B, make_subset(&[(OUT, HB)]));
        assert!(result.verified, "legacy cache key bypasses upstream verification");
    ...
    fn legacy_rejected_inside_nested_model() {
        ...
        let tm = TrustModel::Threshold(2, vec![TrustModel::KeyLegacy(K_CACHE), TrustModel::Key(K1)]);
        let facts = Facts::new();
        ...
        let tm = TrustModel::Threshold(
            1,
            vec![TrustModel::Threshold(1, vec![TrustModel::KeyLegacy(K_CACHE)])],
        );
        assert!(Verifier::new(&facts, &tm).is_err());
    }
